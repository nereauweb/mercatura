...
`ImportProductsJob` (daily) and `ImportPrintingsJob` (monthly) run the same
...
`app:DisableWrongProducts`, `scout:import`, `cleanup:printing_variants`).
...
and `job:import-printings`): `download_data`, `update_live`,
`process_product_data`, `full_products_update`, `update_categories`,
`process_print_data`, `process_source` (`all` or a connector key).
