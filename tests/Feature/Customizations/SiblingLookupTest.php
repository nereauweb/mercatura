...
final class SiblingLookupTest extends TestCase
{
    ...
    public function test_equivalent_option_on_another_variant_and_fallback_to_self(): void
    {
        ...
        $this->assertSame($f->screenOneColorB->id, $f->screenOneColorA->sibling($f->b->id)->id);
        $this->assertSame($f->screenOneColorA->id, $f->screenOneColorB->sibling($f->a->id)->id);
        $this->assertSame($f->screenOneColorA->id, $f->screenOneColorA->sibling($f->a->id)->id, 'own variant');
        $this->assertSame($f->embroideryOptionA->id, $f->embroideryOptionA->sibling($f->b->id)->id, 'no embroidery on B: the option itself is used');
        $this->assertSame($f->screenOneColorA->id, $f->screenOneColorA->sibling(999999999)->id, 'unknown variant: the option itself');
    }
    ...
    public function test_print_price_tier_follows_the_line_quantity_not_the_article_quantity(): void
    {
        ...
        $price = $f->screenOneColorA->calculate_print_price(100, 40, true, false, 30.0);
        ...
        $original = $f->screenOneColorA->calculate_print_price(100, 40, true, true);
        ...
        $stored = $f->screenOneColorA->calculate_print_price(10, 10);
        $this->assertSame(9.99, $stored['unit_price'], 'without a markup the stored (import-time) price is used');

        $this->assertSame('FRONTE - Serigrafia  10x10 1 colore', $f->screenOneColorA->printing_label());
