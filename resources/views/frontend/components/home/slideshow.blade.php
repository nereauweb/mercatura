...
@php
    ...
    $token = fn ($name) => $textTokens[$name] ?? 'text-text';
@endphp
@if(count($slides))
    ...
    @foreach($slides as $slide)
        <div @if(!$loop->first) x-cloak @endif x-show="index === {{ $loop->index }}" x-transition.opacity.duration.500ms class="absolute inset-0" style="background-color: {{ $slide->background_color }}" role="group" aria-roledescription="slide" aria-label="{{ $loop->iteration }} / {{ count($slides) }}">
            @if($slide->background_image)
                <img src="/storage/home_slides/{{ $slide->background_image }}" alt="{{ trim(strip_tags($slide->title_text.' '.$slide->subtitle_text)) }}" width="1200" height="420" class="hidden h-full w-full object-cover sm:block" @if($loop->first) fetchpriority="high" decoding="sync" @else loading="lazy" decoding="async" @endif>
            @endif
            <div class="absolute left-0 top-0 max-w-lg p-6 md:p-8">
                <p class="text-xl font-bold uppercase leading-tight {{ $token($slide->subtitle_color) }}">{!! $slide->subtitle_text !!}</p>
                <p class="mt-1 text-3xl font-bold uppercase leading-tight md:text-4xl {{ $token($slide->title_color) }}">{!! $slide->title_text !!}</p>
                @if($slide->text)<p class="mt-2 text-base font-semibold {{ $token($slide->text_color) }}">{!! $slide->text !!}</p>@endif
            </div>
