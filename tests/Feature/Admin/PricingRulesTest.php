...
class PricingRulesTest extends TestCase
{
    ...
    {
        $band = ProductMarkup::query()->where('condition_3', ProductMarkup::SERIES_STOREFRONT)->where('condition_1', 600)->firstOrFail();
        $this->assertSame(30, (int) $band->value);

        $page = Livewire::test(ManageProductMarkups::class);
        $page->assertOk()->assertSee(__('admin.pricing.series_storefront'));
        $page->callTableAction(EditAction::class, $band, ['condition_3' => 1, 'condition_1' => 600, 'condition_2' => 1000, 'value' => 40])->assertHasNoActionErrors();
        ...
        Livewire::test(ManageProductMarkups::class)->callAction(TestAction::make('create')->table(), data: ['condition_3' => 1, 'condition_1' => 900, 'condition_2' => 1200, 'value' => 33])->assertHasActionErrors(['condition_2']);
        Livewire::test(ManageProductMarkups::class)->callAction(TestAction::make('create')->table(), data: ['condition_3' => 1, 'condition_1' => 100000000, 'condition_2' => 200000000, 'value' => 5])->assertHasNoActionErrors();
        $this->assertSame(1, ProductMarkup::query()->where('condition_1', 100000000)->count());
        Livewire::test(ManageProductMarkups::class)->callAction(TestAction::make('create')->table(), data: ['condition_3' => 1, 'condition_1' => 50, 'condition_2' => 40, 'value' => 5])->assertHasActionErrors(['condition_2']);
    ...
    public function test_changing_a_band_changes_the_configurator_total_at_once(): void
    {
        ...
        $band = ProductMarkup::query()->where('condition_3', ProductMarkup::SERIES_STOREFRONT)->where('condition_1', 600)->firstOrFail();
        Livewire::test(ManageProductMarkups::class)->callTableAction(EditAction::class, $band, ['condition_3' => 1, 'condition_1' => 600, 'condition_2' => 1000, 'value' => 40])->assertHasNoActionErrors();
