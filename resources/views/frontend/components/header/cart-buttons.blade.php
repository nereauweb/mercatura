...
<div {{ $attributes->merge(['class' => 'flex items-center overflow-hidden rounded-full border border-white/30 text-sm']) }}>
    ...
    <a href="{{ route('frontend.quotation.show') }}" title="{{ __('frontend.nav.quotation') }}" @if($quickModal) x-data @click.prevent="$dispatch('quick-quote-open')" @endif class="flex items-center gap-1 border-l border-white/30 px-3 py-1 {{ $quoteCount > 0 ? 'bg-accent text-on-accent' : 'bg-white/10 text-on-dark' }} hover:bg-accent hover:text-on-accent">
        <span class="font-semibold" data-quote-count>{{ $quoteCount }}</span>
