...
/** Product attributes (the ids used by config mercatura.catalog.attributes). Rows in use cannot be deleted. */
final class ProductAttributeResource extends Resource
{
    ...
    {
        return __('admin.catalog.attribute');
    }

    public static function getPluralModelLabel(): string
    {
        return __('admin.catalog.attributes');
    }

    public static function form(Schema $schema): Schema
    {
        return $schema->columns(2)->components([
            TextInput::make('label')->label(__('admin.catalog.label'))->required()->maxLength(64),
            TextInput::make('alias')->label(__('admin.catalog.alias'))->maxLength(64),
        ]);
    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('alias')->label(__('admin.catalog.alias'))->placeholder('-'),
                TextColumn::make('variants_count')->label(__('admin.catalog.in_use_count'))->sortable(),
            ])
            ->recordActions([
                EditAction::make()->modalWidth('lg'),
                DeleteAction::make()->before(function (DeleteAction $action, ProductAttribute $record): void {
    ...
    {
        return [
            'index' => \App\Filament\Resources\Taxonomy\Pages\ManageProductAttributes::route('/'),
        ];
    }
}
